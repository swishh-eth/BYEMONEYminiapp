'use client';

import { TOKEN, SOCIALS, DEXSCREENER } from '@/lib/constants';

const links = [
  {
    id: 'dexscreener',
    name: 'DexScreener',
    description: 'Charts & Trading',
    url: DEXSCREENER.tokenUrl,
  },
  {
    id: 'basescan',
    name: 'Basescan',
    description: 'Contract Explorer',
    url: SOCIALS.basescan,
  },
  {
    id: 'twitter',
    name: 'Twitter / X',
    description: 'Latest Updates',
    url: SOCIALS.twitter,
  },
  {
    id: 'telegram',
    name: 'Telegram',
    description: 'Community Chat',
    url: SOCIALS.telegram,
  },
];

// SVG Icons for links
const LinkIcons: Record<string, JSX.Element> = {
  dexscreener: (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      <path d="M3 3v18h18" />
      <path d="M18 17V9M13 17V5M8 17v-3" />
    </svg>
  ),
  basescan: (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      <circle cx="11" cy="11" r="8" />
      <path d="m21 21-4.35-4.35" />
    </svg>
  ),
  twitter: (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  ),
  telegram: (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
    </svg>
  ),
};

export default function LinksPage() {
  return (
    <div className="flex flex-col h-full overflow-y-auto p-3 gap-3">
      {/* Header */}
      <div className="text-center py-2">
        <h2 className="font-bold text-lg text-white">Official Links</h2>
        <p className="text-xs text-white/50">Connect with ${TOKEN.symbol}</p>
      </div>
      
      {/* Links List */}
      <div className="space-y-2">
        {links.map((link) => (
          <a
            key={link.id}
            href={link.url}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 bg-black border border-white/10 rounded-xl p-3 hover:border-white/20 transition-colors"
          >
            <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center text-white/70">
              {LinkIcons[link.id]}
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-white text-sm">{link.name}</h3>
              <p className="text-[11px] text-white/40">{link.description}</p>
            </div>
            <svg className="w-4 h-4 text-white/30" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
              <path d="M9 5l7 7-7 7" />
            </svg>
          </a>
        ))}
      </div>
      
      {/* Buy Button */}
      <a
        href={`https://app.uniswap.org/swap?outputCurrency=${TOKEN.address}&chain=base`}
        target="_blank"
        rel="noopener noreferrer"
        className="bg-red-500 hover:bg-red-600 rounded-xl p-3 text-center font-bold text-white text-sm transition-colors"
      >
        Buy ${TOKEN.symbol}
      </a>
      
      {/* Share Section */}
      <div className="bg-black border border-white/10 rounded-xl p-3 text-center">
        <p className="text-xs text-white/50">Share this miniapp</p>
        <p className="text-[10px] text-white/30 mt-1">Cast the link to share on Farcaster</p>
      </div>
    </div>
  );
}
